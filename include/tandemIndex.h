...
class TandemIndex {
    public:
        ...
        // Variable-payload API. Inserts the bytes into the value pool and stores
        // the returned handle as the entry's Val_t. lookup returns the payload
        // pointer (owned by the pool) and writes its length to *outLen.
        bool insertPayload(Key_t key, const void *payload, uint32_t len);
        const void *lookupPayload(Key_t key, uint32_t *outLen);
#endif
